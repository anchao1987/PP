 ...
 *    boundary condition, the compute will be implented by parallize.
 *
 *
 *        Version:  1.0
...
//	double rr1,rr2,rr3;
	...
	for(j=0;j<6;j++)
	{
		//分配内存
		//先分配指针单元
		u=(double **)malloc((msize+2)*sizeof(double *));
		hg=(double **)malloc((msize)*sizeof(double *));
		//在分配字符单元
		for(i=0;i<n;i++)
		{
		...
		dx=(double *)malloc(msize*sizeof(double));		

		compute(u,hg,x,dx,msize);
...
void  compute(double **u,double **hg, double *x,double *dx,int n){
	...
	//the cflc number
	double cflc;
	/*the terminal mytime:TERMINATION;
	*the TERMINATION can't be too large since the exact solution is e^{-t \pi^2}
	* *sin(x\pi),If t is too large ,the solution will be very very small.
	*/
	// the maximum number of mytime steps:kcmax
	//int kcmax = 1000000;
	double xlen = (double) (END-BEGIN)/numprocs;
	double xleft = rank*xlen;
	double dxuni;
	//double temp[n+1];
	double *temp;
	int i,j;
...
//		temp[i]=temp[i]+0.1*dxuni*(((double)rand())/RAND_MAX-0.5);
	...
	for(i=0;i<n;i++)
		for(j=0;j<mo;j++)
			u[i+1][j]=ma[j]*((sin(pi*(x[i]+(-0.5)*dx[i]))*fle(j,-0.5)+sin(pi*(x[i]+0.5*dx[i]))*fle(j,0.5))*(1.0/30.0)+(sin(pi*(x[i]+(-0.142615758)*dx[i]))*fle(j,-0.142615758)+sin(pi*(x[i]+0.142615758*dx[i]))*fle(j,0.142615758))*0.277429189+(sin(pi*(x[i]+(-0.382527662)*dx[i]))*fle(j,-0.382527662)+sin(pi*(x[i]+0.382527662*dx[i]))*fle(j,0.382527662))*0.189237478);
	...
	//printf("%f\n",dtime);
	if(mytime+dtime > TERMINATION)
		dtime=TERMINATION-mytime;
	prev = (rank-1+numprocs)%numprocs;
	next = (rank+1)%numprocs;
	while(mytime <= TERMINATION)
	{
		MPI_Send(u[n],mo,MPI_DOUBLE,next,99,MPI_COMM_WORLD);
		MPI_Recv(u[0],mo,MPI_DOUBLE,prev,99,MPI_COMM_WORLD,&status);
		rk(u,hg,x,dx,n);
		mytime+=dtime;
	}
	...
	for(i=0;i<n;i++)
		for(j=0;j<41;j++)
		{
			yy=fabs(eval(u[i+1],(j-20.0)/40.0)-sin(pi*(x[i]+((j-20.0)/40.0)*dx[i]-mytime)));
	...
	ep=fopen(ename,"a+");
	MPI_Reduce(&error1,&er1,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Reduce(&error2,&er2,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Reduce(&error3,&er3,1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);
	//根进程
	if(rank == 0){
		er1 = er1/41.0/2.0;
		er2 = sqrt(er2/41.0/2.0);
...
void rk(double **u,double **hg,double *x,double *dx,int n)
{
	...
	for(i=0;i<n;i++)
		for(j=0;j<mo;j++)
		{
			u[i+1][j]=v[i][j]+0.5*dtime*hg[i][j];
			temp1[i][j]=u[i+1][j];
		}
	for(j = 0; j < mo; j++)
		u[0][j]=u[n][j];
	...
	for(i=0;i<n;i++)
		for(j=0;j<mo;j++)
			u[i+1][j]=(-v[i][j]+temp1[i][j]+2*temp2[i][j]+temp3[i][j]+
...
void res(double **u,double **hg,double *x,double *dx,int n)
{
	...
	if(mp > 0)
			...
			{
				if(mp==1)
					h[i][1]=h[i][1]+(flux(eval(u[i+1],-0.5))+
							flux(eval(u[i+1],0.0))*4.0+flux(eval(u[i+1],0.5)))/6.0;
				else if(mp==2)
					for(k=1;k<mo;k++)
						h[i][k]=h[i][k]+((flux(up[i])*fled(k,-0.5)+flux(un[i])*fled(k,0.5))*(1.0/12.0)+
							(flux(eval(u[i+1],-sqrt(5.0)/10.0))*fled(k,-sqrt(5.0)/10.0)+
						flux(eval(u[i+1],sqrt(5.0)/10.0))*fled(k,sqrt(5.0)/10.0))*(5.0/12.0));
				else if(mp==3)
					for(k=1;k<mo;k++)
						h[i][k]=h[i][k]+((flux(up[i])*fled(k,-0.5)+flux(un[i])*fled(k,0.5))*0.05+
						(flux(eval(u[i+1],-sqrt(21.0)/14.0))*fled(k,-sqrt(21.0)/14.0)+
							flux(eval(u[i+1],sqrt(21.0)/14.0))*fled(k,sqrt(21.0)/14.0))*(49.0/180.0)+
						(flux(eval(u[i+1],0))*fle(k,0))*(16.0/45.0));

				else
					for(k=1;k<mo;k++)
						h[i][k]=h[i][k]+((flux(up[i])*fled(k,-0.5)+flux(un[i])*fled(k,0.5))*(1.0/30.0)+(flux(eval(u[i+1],sqrt(147.0-42.0*sqrt(7.0))/42.0))*fled(k,sqrt(147.0-42.0*sqrt(7.0))/42.0)+flux(eval(u[i+1],-sqrt(147.0-42.0*sqrt(7.0))/42.0))*fled(k,-sqrt(147.0-42.0*sqrt(7.0))/42.0))*((14.0+sqrt(7.0))/60.0)+(flux(eval(u[i+1],sqrt(147.0+42.0*sqrt(7.0))/42.0))*fled(k,sqrt(147.0+42.0*sqrt(7.0))/42.0)+flux(eval(u[i+1],-sqrt(147.0+42.0*sqrt(7.0))/42.0))*fled(k,-sqrt(147.0+42.0*sqrt(7.0))/42.0))*((14.0-sqrt(7.0))/60.0));
			}
	for(i=0;i<n;i++)
		for(k=0;k<mo;k++)
